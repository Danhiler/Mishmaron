<%@ Page Language="C#" MasterPageFile="~/MasterPage2.master" AutoEventWireup="true" CodeFile="Shifts_next_week.aspx.cs" Inherits="Shifts_next_week" Title="סידור עבודה - שבוע הבא" %>
...
var Is;
    ...
    {
     %>
Is=window.open("Add_Intermediate_Shift.aspx?Day="+txtArea+"&IS="+Trans_To_english(str.value)+"&name="+Trans_To_english("<%if(use_jobs){Response.Write(""+short_job+"|"+Session["Wname"].ToString()+"");}else{Response.Write(""+Session["Wname"].ToString()+"");} %>"),""+txtArea+"","menubar=no,resizable=yes,status=no,scrollbars=no,directories=no,toolbar=no, height=170, width=330, alwaysRaised=yes, left=400")
...
return confirm("האם אתה בטוח שברצונך לבצע פעולה זאת");
     ...
     {
     window.location="http://www.mishmaron.com/Publish_Shifts.aspx?week=1";
     }
     function change_location_s()
     {
     window.location="http://www.mishmaron.com/Switchers_Page.aspx?week=1";
     }
</script>
    <div>
    ...
    <tr><td style="width: 30px;">
      <a href="Shifts_this_week.aspx"><img src="Pictures\right_arrow.png" width="30" border="0" alt="שבוע הבא" /></a>
      </td><td style="text-align: center">
      <%    if (Session["Admin"] != null && (bool)Session["Admin"])
            {
                Response.Write("<a href='Print_Mode.aspx?id=" + un + "&name=" + trance_name + "&week=1' target='_blank' >גרסת הדפסה</a>");
                Response.Write("&nbsp;&nbsp;&nbsp;<a href='Requests_Page.aspx?week=1' target='_blank' >טבלת בקשות</a>");
            }
        %>
      </td>
          <td style="width: 30px;">
      <img src="Pictures\left_arrow_dis.png" width="30" border="0" alt="" />  
      </td>
                                                                           </tr>
      </table>   

    <input type="hidden" name="Week" value="1"/>
        <table width=" 80%" id="Shifts_Table" class="sample" >
            <tr>
                <td>
                    &nbsp;</td>
...
<% 
   ...
   {
        %>
        <td style="width: 35px"><input type="submit" id="m_button" name="Erase_All" value="מחק הכל" /></td>
       <%if (shifts_ready)
         {
       ...
       <%if (!shifts_ready) Response.Write("<td style='width: 300px' ><input style='height:30px' id='m_button' type='submit' name='AI_shift' value='סדר משמרות' /></td>"); %>
        ...
        <td style="text-align: left"><input class="button" id="m_button" type="button" value="הראה עובדים" onclick="open_show_workers()" name="show_workers" /></td>
