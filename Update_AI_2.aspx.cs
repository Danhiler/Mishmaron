    ...
    {//first number: 0=>mornming 1=>intermediate 2=>evening
        ...
        if(dt.Rows[0]["morning"].ToString()!="")
            for (int i = 0; i < shifts.Length; i++)
            {
                ...
                if (int.Parse(param[0]) > 3)//second page include only 3 last days
                {
        ...
        for (int i = 4; i < 7; i++)
        {
            if (i == 6) Response.Write("<td align='center' colspan='2'>");
        ...
        for (int i = 4; i < 7; i++)
        {
            ...
            if (!use_jobs)
                for (int count = 0; count < 3; count++)
                {
                    ...
                    for(int j =0;j<4;j++)
                    {
                        Response.Write("<td><input name='H_" + i + "_" + count + "_" + j + "' maxlength='2' size='2' type='text' style='width: 15px' value='" + GetHour(shifts_in_days[i, count].hours,j) + "' /></td>");
                        if(j!=3)
